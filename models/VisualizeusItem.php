 ...
 *  
 */
class VisualizeusItem extends SourceItem implements ImageItem {
	...
	public function setTitle($title) {
		...
		$sql = "UPDATE `visualizeus_data` SET `title`=:title "
	...
	public function setDescription($note) {
		$db = Zend_Registry::get('database');
		$sql = "UPDATE `visualizeus_data` SET `note`=:note "
			 . "WHERE source_id = :source_id AND id = :item_id ";
		$data 		= array("source_id" 	=> $this->getSource(),
							"item_id"		=> $this->getID(),
							"note"			=> $note);
	...
	public function getImageUrl($size=ImageItem::SIZE_MEDIUM) {
		...
		return str_replace("_m.jpg","$size.jpg",$this->_data['img_url']);
	}
